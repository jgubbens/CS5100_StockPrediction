...
class TradingEnv_5m(gym.Env):
    def __init__(self, df_5m, df_4h, q_table_4h):
        ...
        # action space [0: call, 1: short, 2: hold, 3: close position]
        self.action_space = spaces.Discrete(4)

        # observation: [resistance_diff_pct, support_diff_pct, volume%, current position, 4h_decition]
        low = np.array([-np.inf, -np.inf, 0, -1] + [0, 0, 0, 0], dtype=np.float32)
        high = np.array([np.inf, np.inf, 1, 1] + [1, 1, 1, 1], dtype=np.float32)
        ...
        self.take_profit_pct = 0.1 # Target Price at +10%
    ...
    def discretize_state(self, state, bins=[10, 10, 10, 3]):
        # bucket
    ...
    def _predict_4h_action(self, timestamp):
        ...
        if state_d in self.q_table_4h:
            action = np.argmax(self.q_table_4h[state_d])
        else:
            action = 2
        one_hot = np.zeros(4)
        one_hot[action] = 1
    ...
    def _get_observation(self):
        ...
        obs = np.concatenate([[support_pct, resistance_pct, volume_pct, position], pred_4h]).astype(np.float32)
        return obs
